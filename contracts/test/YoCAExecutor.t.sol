...
contract YoCAExecutorTest is Test {
    YoCAExecutor public dca;
    ...
    function setUp() public {
        ...
        ERC1967Proxy proxy = new ERC1967Proxy(
            address(impl),
            abi.encodeCall(YoCAExecutor.initialize, (owner))
        );
        dca = YoCAExecutor(address(proxy));

        usdc.mint(address(yoUSD), 1_000_000 * 1e6);
        weth.mint(address(yoETH), 1000 * 1e18);

        router.setRate(address(yoUSD), address(yoETH), 5, 10000);
    ...
    function test_ExecuteDCA_Success() public {
        ...
        uint256 expectedOut = (amountIn * 5) / 10000;
    ...
    function test_ExecuteDCA_RevertIfNotKeeper() public {
        ...
        vm.prank(user);
        vm.expectRevert(YoCAExecutor.OnlyKeeper.selector);
        dca.executeDCA(
            user,
            address(yoUSD),
            address(yoETH),
    ...
    function test_ExecuteDCA_RevertIfRouterNotAllowed() public {
        ...
        vm.prank(keeper);
        vm.expectRevert(YoCAExecutor.RouterNotAllowed.selector);
        dca.executeDCA(
            user,
            address(yoUSD),
            address(yoETH),
    ...
    function test_ExecuteDCA_RevertIfMinAmountOutNotMet() public {
        ...
        vm.prank(keeper);
        vm.expectRevert(YoCAExecutor.InsufficientOutput.selector);
        dca.executeDCA(
            user,
            address(yoUSD),
            address(yoETH),
    ...
    function test_SetKeeper_Success() public {
        ...
        emit YoCAExecutor.KeeperUpdated(keeper, newKeeper);
    ...
    function test_SetRouterAllowed_Success() public {
        ...
        emit YoCAExecutor.RouterUpdated(address(router), false);
        dca.setRouterAllowed(address(router), false);
